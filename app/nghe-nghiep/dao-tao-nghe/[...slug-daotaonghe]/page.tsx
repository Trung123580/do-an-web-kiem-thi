...
export default function page() {
    ...
    const slug: any = params["slug-daotaonghe"]
    const daotaoDetail: any = daotaonghe.find((daotao) => daotao.slug === slug[0]);
        ...
        <>
            ...
            <div className="bg-[#D8E0FB] px-4">
                <div className="py-14 max-w-7xl mx-auto relative">
                    <div className="mb-6">
                        {daotaoDetail?.organization.map((item: any, index: number) => (
                    ...
                    <h1 className="text-5xl font-bold mb-4">{daotaoDetail?.title}</h1>
                    <p className="mb-16 text-lg max-w-4xl">{daotaoDetail?.description}</p>
                    <div className="bg-[#FFFFFF] rounded-xl shadow-sm p-8 absolute -bottom-75 sm:-bottom-36 md:-bottom-20 xl:-bottom-15 left-1/2 -translate-x-1/2  w-full">
                        <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-5 gap-4">
                            <div className="flex justify-center gap-1 flex-col px-5 sm:border-r border-[#CFD1D4]">
                                ...
                                <span className="text-2xl">{daotaoDetail?.time}</span>
                            </div>
                            <div className="flex justify-center gap-1 flex-col px-5 sm:border-r border-[#CFD1D4]">
                                <span className="text-sm text-[#70747D]">Khóa học</span>
                                <span className="text-2xl">{daotaoDetail?.form}</span>
                            </div>
                            <div className="flex justify-center gap-1 flex-col px-5 sm:border-r border-[#CFD1D4]">
                                <span className="text-sm text-[#70747D]">Hình thức</span>
                                <span className="text-2xl">Trực tiếp</span>
                            </div>
                            <div className="flex justify-center gap-1 flex-col px-5 sm:border-r border-[#CFD1D4]">
                                <span className="text-sm text-[#70747D]">Chi phí buổi học</span>
                                <span className="text-2xl">{daotaoDetail.free ? 'Miễn phí' : daotaoDetail?.price}</span>
                            </div>
                            <div className="flex justify-center gap-1 flex-col px-5">
                                <span className="text-sm text-[#70747D]">Độ tuổi</span>
                                <span className="text-2xl">{daotaoDetail?.objectives}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="min-h-screen pb-16 py-16 pt-88 sm:pt-48 md:pt-30 xl:pt-32 max-w-7xl mx-auto px-4">
                <div className="container mx-auto">
                    ...
                    <div className="text-base" dangerouslySetInnerHTML={{ __html: daotaoDetail?.courseIntroduction ?? "" }} />
                    <h2 className="text-[32px] font-bold text-[#2E4FBE] mb-2 mt-10">Đầu ra</h2>
                    <div className="text-base" dangerouslySetInnerHTML={{ __html: daotaoDetail?.output ?? "" }} />
                    <h2 className="text-[32px] font-bold text-[#2E4FBE] mb-2 mt-10">Nội dung khóa học</h2>
                    <div className="text-base" dangerouslySetInnerHTML={{ __html: daotaoDetail?.content ?? "" }} />
                    ...
                    <div className="gap-3.5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                        {daotaoDetail?.commnents.map((item: any, index: number) => (
                    ...
                    <div className="flex flex-col mt-18 gap-3.5 bg-[#F2F2F3] py-12 px-[72px] rounded-lg">
                        ...
                        <div className="text-base" dangerouslySetInnerHTML={{ __html: daotaoDetail?.scienceRegistration ?? "" }} />
                    </div>
                </div>
            </div >
        </>
